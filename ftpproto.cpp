...
{
    int ret;
   	ftp_reply(sess, FTP_GREET, "(ftpd v0.1.0)");
    while(1)
    {
...
int pasv_active(session_t *sess)
{
	//由于是nobody创建的监听套接字，sess->pasv_listen_fd在nody中赋值，
	//不能直接判断ftp进程sess->pasv_listen_fd
...
static void do_syst(session_t *sess)
{
    ftp_reply(sess,FTP_SYSTOK,"UNIX_Type:L8");
}
static void do_feat(session_t *sess)
{
